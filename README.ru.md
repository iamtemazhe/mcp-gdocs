...
Большинство операций записи принимают массив `items` для выполнения нескольких действий за один вызов.
...
| `docs_replace_with_markdown` | Замена содержимого документа из Markdown |
...
| `drive_move_file` | Перемещение одного или нескольких файлов |
...
### Service Account vs OAuth — что выбрать?
...
### Способ A: Service Account (для серверов и автоматизации)
...
**A1. `SERVICE_ACCOUNT_PATH` — путь к JSON-ключу (+ impersonation)**
...
`GOOGLE_IMPERSONATE_USER` опционален — нужен для Google Workspace с domain-wide delegation.

**A2. `CREDENTIALS_CONFIG` — Base64-encoded JSON (Docker / CI / K8s)**

Удобен в окружениях, где управлять файлами сложно, а переменные окружения — легко.
...
**A3. `GOOGLE_APPLICATION_CREDENTIALS` — стандартная переменная Google**
...
### Способ B: OAuth 2.0 (для личной работы)

Для личного использования, где интерактивный вход через браузер допустим.
...
- **Безопасный диапазон стилей абзаца:** `docs_apply_paragraph_style` и `docs_apply_heading_style` автоматически сдвигают `startIndex` на +1 (если > 1), чтобы стили не затрагивали предыдущий абзац. Это связано с поведением Google Docs API, где граница структурного элемента совпадает с символом `\n` предыдущего абзаца.
