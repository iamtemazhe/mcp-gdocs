...
- Bundle Docs + Sheets + Slides into one oversized server

## What mcp-gdocs gives you

### 1. Complete document control in one server

44 tools covering the full Google Docs + Drive lifecycle, with multi-tab support:
...
All write, format, comment, and drive tools accept multiple items per call. One tool call can perform dozens of operations — formatting 50 headings, inserting 20 text blocks, or creating 10 Drive files in a single request.
...
### 7. Five authentication methods

OAuth, Service Account (file path), Base64 credentials (for Docker/CI), Application Default Credentials (for GKE/Cloud Run), and Impersonation. Multi-profile support for isolated token management. Other implementations typically support only two authentication methods.
...
| Unified batch API | 10 operation types in one call |
...
| 5 auth methods | OAuth, SA, Base64, ADC, Impersonation |
